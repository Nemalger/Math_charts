...
class GameApp(App):
    ...
    def update(self, dt):
        self.time += dt
        self.root.ids['MapOfPlanets_window'].ids['clock'].text = 'time:' f"{self.time: .0f}"
        self.root.current_screen.update(dt)
        # self.root.ids['PlanetInfo'].ids['food_manufacture'].text = str(int(phys.get_function('food', t = self.time, p = int(self.root.ids['PlanetInfo'].ids['food/humans'].value), P= int(self.root.ids['game_window'].ids['humans_amount'].text))))
